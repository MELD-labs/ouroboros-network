...
-- | Like roundtrip', but checks for equality on of the encoding (i.e. the byte
...
roundtrip_SerialiseNodeToClient ccfg =
    ...
    -- require an Eq and Show instance for all ledger config types which we'd
    -- like to avoid.
    , testProperty ("roundtrip (comparing encoding) LedgerConfig") $ forAllShow
        arbitrary
        (const "LedgerConfig {..}")
