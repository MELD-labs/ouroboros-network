...
mistake in an old entry, we address it by adding new lines at the top of the log
entry. The new list item within the old entry should follow this template `-
...
entry; crucially, the the code relevant to that entry is not changing. Such code
...
- On-the-wire content of Alonzo transactions. Previously they were sent across
  ...
  Beyond the original motivation, it is also useful to receive this flag
