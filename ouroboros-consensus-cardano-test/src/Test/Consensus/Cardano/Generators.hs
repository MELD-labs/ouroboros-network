...
-- | When it comes to roundtrip tests for HFC, all eras must be enabled. This is
-- because the encoder omits disabled eras and the decoder expects all eras to
-- be present in the serialisation. See @encodeNodeToClientNP@ and
...
instance (CanMock (ShelleyEra c), CardanoHardForkConstraints c)
  ...
  arbitrary = WithVersion
    -- Assume that all eras are enabled in the maximum supported version.
    (snd $ fromJust $ Map.lookupMax $ supportedNodeToClientVersions (Proxy @(CardanoBlock c)))
    <$> arbitrary
