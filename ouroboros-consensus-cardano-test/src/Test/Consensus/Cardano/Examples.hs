    ...
    arbitraryPartialShelleyLedgerConfig translationContext = ShelleyLedgerConfig {
      ...
      where
        ...
        genesis = ShelleyGenesis {
          ...
          , sgNetworkMagic      = 1
          , sgNetworkId         = Mainnet
          , sgActiveSlotsCoeff  = fromJust (boundRational 0.2)
          , sgSecurityParam     = 3
          ...
          , sgProtocolParams    = PParams {
              ...
              , _eMax            = 170
              , _nOpt            = 18
              , _a0              = fromJust (boundRational 0.19)
              , _rho             = fromJust (boundRational 0.21)
              , _tau             = fromJust (boundRational 0.22)
              , _d               = fromJust (boundRational 0.23)
          ...
          -- `sgInitialFunds` is overwritten with `Map.empty` by `compactGenesis`
          , sgInitialFunds      = Map.empty
          -- `sgStaking` is overwritten with `emptyGenesisStaking` by `compactGenesis`
